globals     [x-start y-start] 
patches-own [state]

to setup
  clear-all
  set x-start 0.0
  set y-start 0.8 * max-pycor
  ask patches [set pcolor 96 set state 0]
  ask patches with [pycor = min-pycor or pxcor = min-pxcor or pxcor = max-pxcor] [set pcolor black]
  ask patches with [pycor > y-start] [set pcolor 97]
  ask patches with [pcolor = black]  [set state 1]
end

to go
  ifelse all? (patch-set patch (x-start + 1.5) (y-start + 1.5) patch (x-start + 1.0) (y-start + 1.0)) [count turtles-here > 10] 
    [ if auto-move? [set x-start x-start + 1]] ;;this moves the source to the right as the flow stops
    [ create-turtles 1 
      [ ifelse (random 100 < percent-red)
        [ set color red   set shape "square" set size 2 setxy (x-start + 1.5) (y-start + 1.5)] 
        [ set color white set shape "circle" set size 1 setxy (x-start + 1.0) (y-start + 1.0)]
      set heading 180]
    repeat 3 [move-all] ] 
end
...
to move-all
  ask turtles 
    [ifelse color = white ;sand particles
      [ifelse ([state] of patch-at 0 -1 = 0) 
        [ ask patch-here [set state 0] forward 1 ask patch-here [set state 1]  ]
        [ if all? (patch-set patch-at 1 0 patch-at 1 -1) [state = 0]
          [ ask patch-here [set state 0] left 90 forward 1 right 90 forward 1 ask patch-here [set state 1] ]]]
      ;sugar particles
      [ifelse all? (patch-set patch-at 0 -2 patch-at -1 -2) [state = 0] 
        [ ask (patch-set patch-here patch-at -1 0)     [set state 0]    ;the bottom two will be changed next
          forward 1
          ask (patch-set patch-at 0 -1 patch-at -1 -1) [set state 1] ]  ;the top two were never changed  
        [ if all? (patch-set patch-at 1 0 patch-at 1 -1 patch-at 1 -2 patch-at 2 0 patch-at 2 -1 patch-at 2 -2) [state = 0] 
          [ ask (patch-set patch-here patch-at -1 0 patch-at -1 -1 patch-at 0 -1) [set state 0] ;
            left 90 forward 1 right 90 forward 1
            ask (patch-set patch-here patch-at 0 -1 patch-at -1 -1) [set state 1] ] ] ] ]      
end
;CopyRight 2014 Vic Castillo under a Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.
...
193
538
The auto-move? switch will allow the source to automatically move to the right.
11
0.0
1
...
162
380
See the THINGS TO TRY section in the info tab\n
11
0.0
1
...
![screenshot](https://raw.githubusercontent.com/VicCastillo/SpontaneousStratification/master/SpontaneousStratification-Sreenshot.PNG)
